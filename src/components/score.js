...
export default function Score({ username, index }) {
    const [scoreValue, setScoreValue] = useState(JSON.parse(localStorage.getItem(username))?.[index] || 0)

    useEffect(() => {
        const likes = JSON.parse(localStorage.getItem(username))
        likes[index] = scoreValue
        localStorage.setItem(username, JSON.stringify(likes))
    }, [scoreValue])

    function handleClickLower() {
        setScoreValue((oldScoreValue) => oldScoreValue - 1)
    }
    ...
    return(
        <div className="score-content">
            <button onClick={handleClickLower} disabled={scoreValue === 0} className='dislike-button'>
                <svg 
            ...
            <button onClick={handleClickHigher} disabled={scoreValue === 10} className='like-button'>
                <svg 
                    className='like-icon'
                    role="img"
                    height={35}
